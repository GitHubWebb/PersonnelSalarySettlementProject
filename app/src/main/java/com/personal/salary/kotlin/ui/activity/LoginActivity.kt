...
import com.personal.salary.kotlin.aop.SingleClick
import com.personal.salary.kotlin.app.AppActivity
import com.personal.salary.kotlin.databinding.LoginActivityBinding
import com.personal.salary.kotlin.http.glide.GlideApp
import com.personal.salary.kotlin.ktx.startActivity
import com.personal.salary.kotlin.manager.InputTextManager
import com.personal.salary.kotlin.manager.UserManager
import com.personal.salary.kotlin.other.KeyboardWatcher
import com.personal.salary.kotlin.util.VERIFY_CODE_URL
import com.personal.salary.kotlin.wxapi.WXEntryActivity
...
class LoginActivity : AppActivity(), UmengLogin.OnLoginListener,
        ...
        fun start(context: Context, phone: String?, password: String?) {
            if (context is LoginActivity) {
                context.startActivity<MainActivity> {
                    putExtra(FROM_HOME, (context as? MainActivity) != null)
                    putExtra(INTENT_KEY_IN_PHONE, phone)
                    putExtra(INTENT_KEY_IN_PASSWORD, password)
                }

            } else context.startActivity<LoginActivity> {
                putExtra(FROM_HOME, (context as? MainActivity) != null)
                putExtra(INTENT_KEY_IN_PHONE, phone)
                putExtra(INTENT_KEY_IN_PASSWORD, password)
            }
    ...
    override fun initView() {
        with(mBinding) {
            ...
            commitView?.let {
                InputTextManager.with(this@LoginActivity).addView(phoneView).addView(passwordView)
                    .addView(etLoginVerifyCode).setMain(it).build()
            }
        }
    }
    ...
    override fun initData() {
        ...
        loadVerifyCode()
        postDelayed({
            KeyboardWatcher.with(this@LoginActivity).setListener(this@LoginActivity)
        }, 500)
    ...
    override fun initEvent() {
        phoneView?.doAfterTextChanged { result ->
            ...
            if (RegexUtils.isMobileExact(result)) {
                ...
                Glide.with(this).load(R.mipmap.ic_default_avatar).into(mBinding.ivLoginLogo)
            }
        }
        ...
        with(mBinding) {
            ...
            cbRememberPwd.setOnCheckedChangeListener { _, isChecked ->
                UserManager.isRememberPwd(
                    isChecked
                )
            }
        }
    ...
    private fun loadVerifyCode() {
        GlideApp.with(this).load(VERIFY_CODE_URL).diskCacheStrategy(DiskCacheStrategy.NONE)
            .skipMemoryCache(true).into(mBinding.sivLoginVerifyCode)
    }

    override fun onLeftClick(titleBar: TitleBar) {
        // 用户点击了跳过按钮
        // HomeActivity.start(this, MyMeFragment::class.java)
        start(this, "13763740317", "Aa123456")
        finish()
    }
    ...
    override fun onSucceed(platform: Platform?, data: UmengLogin.LoginData?) {
        ...
        logoView?.let {
            GlideApp.with(this).load(data?.getAvatar()).circleCrop().into(it)
        }
    ...
    override fun onSoftKeyboardOpened(keyboardHeight: Int) {
        ...
        bodyLayout?.let {
            val objectAnimator: ObjectAnimator = ObjectAnimator.ofFloat(
                it, "translationY", 0f, (-(commitView?.height?.toFloat() ?: 0f))
            )
