...
public class AutoExcel {
    private static final String TAG = "AutoExcel";
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.N)
    public static void save(String templatePath, String outputPath, List<TemplateExportPara> templateExportParas, ExcelSetting excelSetting) {
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.N)
    public static void save(String templatePath, String outputPath, List<TemplateExportPara> templateExportParas, Consumer<Workbook> actionAhead, Consumer<Workbook> actionBehind) {
        ...
        if (actionAhead != null) if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
        ...
        if (actionBehind != null) if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            actionBehind.accept(workbook);
        }

        if (forceFormulaRecalculation) workbook.setForceFormulaRecalculation(true);
    ...
    public static void save(String outputPath, List<DirectExportPara> directExportParas) {
        ...
        for (DirectExportPara directExportPara : directExportParas) {
            if (directExportPara.getDataSource() == null) continue;

            if (directExportPara.getSheetName() == null || directExportPara.getSheetName().isEmpty())
                sheet = workbook.createSheet();
            else sheet = workbook.createSheet(directExportPara.getSheetName());
            ...
            if (fieldSettings == null || fieldSettings.size() == 0) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    fieldSettings = fieldNameFields.values().stream().map(m -> new FieldSetting(m.getName(), m.getName())).collect(Collectors.toList());
                }
            }
            int rowIndex = 0;
            int colIndex = 0;
            ...
            for (FieldSetting fieldSetting : fieldSettings) {
                SheetUtil.setValue(sheet, rowIndex, colIndex, fieldSetting.getDisplayName(), exportContext).setCellStyle(exportContext.getDefaultHeadStyle());
            ...
            try {
                if (dataSourceType == DataSourceType.List) {
                    ...
                    writeRecordByFieldSetting(sheet, fieldSettings, directExportPara.getDataSource(), fieldNameFields, rowIndex, exportContext);
                }
            } catch (IllegalAccessException e) {
                throw new AutoExcelException(e);
            }
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.N)
    private static List<BlockNameResolver> resolveCellNames(Workbook workbook, List<TemplateExportPara> paras, ExportContext exportContext) {
        ...
        for (Name name : names) {
            ...
            if (!currDataSourceName.equalsIgnoreCase(arr[0])) {
                ...
                Optional<TemplateExportPara> para = paras.stream().filter(m -> m.getDataSourceName().equalsIgnoreCase(finalCurrDataSourceName)).findFirst();
    ...
    private static void reLocate(List<BlockNameResolver> blockNameResolvers, String currSheetName, ExportContext exportContext) {
        for (BlockNameResolver blockNameResolver : blockNameResolvers) {
            if (!blockNameResolver.getSheetName().equals(currSheetName)) continue;
            ...
            blockNameResolver.getFormulaCellManagers().forEach(formulaCellMgr -> {
                Cell cell = SheetUtil.getOrCreateCell(blockNameResolver.getSheet(), formulaCellMgr.getRowIndex(), formulaCellMgr.getColIndex(), exportContext);
     ...
     */
    private static void writeRecordByCellName(BlockNameResolver blockNameResolver, Object dataSourceRecord, Map<String, Map<String, Field>> dataSourceNameFields, int step, ExportContext exportContext) throws IllegalAccessException {
        for (Map.Entry<String, CellManager> entry : blockNameResolver.getFieldNameCells().entrySet()) {
            ...
            if (dataSourceNameFields.containsKey(dataSourceName) && dataSourceNameFields.get(dataSourceName).containsKey(fieldName)) {
                ...
                if (dataDirection == DataDirection.Down) rowIndex += step;
                else colIndex += step;
     ...
     */
    private static void writeRecordByFieldSetting(Sheet sheet, List<FieldSetting> fieldSettings, Object dataSourceRecord, Map<String, Field> fieldNameFields, int rowIndex, ExportContext exportContext) throws IllegalAccessException {
        int colIndex = 0;
        for (FieldSetting fieldSetting : fieldSettings) {
    ...
    private static void writeAggregate(BlockNameResolver blockNameResolver, int recordCount, ExportContext exportContext) {
        for (String fieldName : blockNameResolver.getFieldNameAggregateCells().keySet()) {
            ...
            Cell aggregateCell = SheetUtil.getOrCreateCell(blockNameResolver.getSheet(), aggregateCellManager.getRowIndex(), aggregateCellManager.getColIndex(), exportContext);
                ...
                switch (aggregateCellManager.getAggregateType()) {
                    case SUM:
                        if (blockNameResolver.getPara().getDataDirection() == DataDirection.Down)
                            aggregateCell.setCellFormula(String.format("SUM(%1$s%2$s:%1$s%3$s)", colName, rowIndex + 1, rowIndex + recordCount));
                        else
                            aggregateCell.setCellFormula(String.format("SUM(%1$s%2$s:%3$s%2$s)", colName, rowIndex + 1, WorkbookUtil.indexToColName(colIndex + recordCount - 1)));
                        break;
                    case AVG:
                        if (blockNameResolver.getPara().getDataDirection() == DataDirection.Down)
                            aggregateCell.setCellFormula(String.format("AVERAGE(%1$s%2$s:%1$s%3$s)", colName, rowIndex + 1, rowIndex + recordCount));
                        else
                            aggregateCell.setCellFormula(String.format("AVERAGE(%1$s%2$s:%3$s%2$s)", colName, rowIndex + 1, WorkbookUtil.indexToColName(colIndex + recordCount - 1)));
                        break;
                }
            }
        }
    ...
    private static void writeFormula(BlockNameResolver blockNameResolver, int step, ExportContext exportContext) {
        ...
        blockNameResolver.getFormulaCellManagers().forEach(formulaCellManager -> {
            ...
            while (matcher.find()) {
                ...
                else replacement = WorkbookUtil.getColName(colName, step) + rowIndex;
    ...
    private static void writeRowNo(BlockNameResolver blockNameResolver, int step, ExportContext exportContext) {
        ...
        SheetUtil.setValue(blockNameResolver.getSheet(), rowNoCellMgr.getRowIndex() + step, rowNoCellMgr.getColIndex(), step + 1, exportContext).setCellStyle(rowNoCellMgr.getCellStyle());
    }
    ...
    public static List<FieldSetting> genProjectFieldSettings(Class clazz) {
        ...
        for (int j = 0; j < fields.length; j++) {
            ...
            try {
                ...
                ExcelProperty column = field.getAnnotation(ExcelProperty.class);
                if (column == null || column.value().length == 0) continue;

                fieldSettings.add(new FieldSetting(field.getName(), column.value()[0]));
            } catch (Exception e) {
                Log.e(TAG, "getImportParas: ", e);
            }
        }
